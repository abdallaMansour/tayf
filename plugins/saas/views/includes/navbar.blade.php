    ...
    @if (auth()->user()->can('Manage SAAS Settings'))
        <li
            ...
            <ul class="nav sub-menu">
                ...
                <li class="{{ Request::routeIs(['plugin.saas.database-credentials.index', 'plugin.saas.database-credentials.create', 'plugin.saas.database-credentials.show', 'plugin.saas.database-credentials.edit']) ? 'active ' : '' }}">
                    <a href="{{ route('plugin.saas.database-credentials.index') }}">{{ translate('Database Credentials') }}</a>
                </li>
            </ul>
        </li>
    @endif
